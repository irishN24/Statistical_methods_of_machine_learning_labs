...
n <- N
skewness_binom <- sum((binom_sample - mean_binom)^3) / (n * sd_binom^3)
kurtosis_binom <- sum((binom_sample - mean_binom)^4) / (n * sd_binom^4) - 3
...
cat(paste("  Коэффициент асимметрии:", round(skewness_binom, 4), "\n"))
cat(paste("  Коэффициент эксцесса:", round(kurtosis_binom, 4), "\n"))
...
# Для геометрического
skewness_geom <- sum((geom_sample - mean_geom)^3) / (n * sd_geom^3)
kurtosis_geom <- sum((geom_sample - mean_geom)^4) / (n * sd_geom^4) - 3
...
cat(paste("  Коэффициент эксцесса:", round(kurtosis_geom, 4), "\n"))
